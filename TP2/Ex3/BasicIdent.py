...
# Define parameters
p = random_prime(2^128)  # Choose a large prime p
Fp = Zmod(p)  # Define finite field Fp

# Create an elliptic curve over the finite field Fp
ec = EllipticCurve(Fp, [0, 1])  # Example curve y^2 = x^3 + 1

# Find a point of finite order on the elliptic curve
P = ec.random_point()  # Random point on the curve

# Define a distortion map
def distortion_map(P):
    return ec.random_point() + P  # Simply adding a random point to P

# Ensure the point has finite order
while P.order() == 1:
    P = ec.random_point()

# Test the distortion map
dmap = distortion_map(P)
